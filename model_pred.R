...
# PURPOSE: run ridge regreession on the training set to derive a prediction model 
# AUTHOR: AB

# CREATED:	20200302
# UPDATED: 	

# INPUT DATA: train.data2				
# OUTPUT: 	
...
##UPDATE: the sbrl package was removed from CRAN and no longer works as of 2020 - 04 - 28

setwd("C:/Users/Barny/Documents/KI_project_4")


#### START of scalable bayesian rule list code
#### using sbrl along with the risk groups low, medium and high to try to predict risk of VL failure
#### the otcome is variable label. Must get rid of all other outcome variables, and also make sure that
#### all covariates are factors

train.sbrl <-train.data3 %>% select(male, age35, age50, age65,
                                    buyamba, kabira, kakuuto, kalisizo, kasaali, kasasa,
                                    kibaale, kifamba, kyebe, lwamaggwa,
                                    lwanda, lyantonde, nabigasa,
                                    basewho1, basewho2, basewho3, basewho4,
                                    ...
                                    reg_D4T3TCNVP,
                                    prev_fail1, prev_fail2, prev_fail3,
                                    lastVL_t, enroll_t, lastVLcat, label) %>% data.frame()


#### changing all variable to factor type
for (name in names(train.sbrl)) {train.sbrl[name] <- as.factor(train.sbrl[,name])}

#### fitting the sbrl model
#####
sbrl_model <- sbrl(train.sbrl, #### this is the training dataset to fit the model
                   iters=20000, #### number of iterations for each MCMC chain
                   pos_sign=c("medium","high"), #### the sign for the positive labels in the label column
                   neg_sign="low", #### the sign for the negative labels in the label column
                   ...
                   minsupport_neg=0.70, #### a number between 0 and 1, for the minimum percentage support for the negative obs
                   lambda=10.0, ### expected length of rule list
                   eta=1.0, #### expected cardinality of the rules in optimal rule list
...
# sbrl_pred <- predict(sbrl_model, train.sbrl)
# 
# 
# ##### putting th predicted probabilites and risk groups together to compare predictive accuracy
# train.sbrl$sbrl_pred <- sbrl_pred$V2
# 
# 
# #### sbrl separated subjects into 4 groups, .04 (58%), .13 (35%), .29 (2.5%), .61 (1.8%) (probabilities of being medium or high risk)
# #### high risk group - 
# sbrl.table1 <- table(train.sbrl$label,train.sbrl$sbrl_pred)
# sbrl.table2 <- as.data.frame.matrix(sbrl.table1)
# 
# ##### turns the contingency table into percentages in a dataframe
# sbrl.table3 <- prop.table(sbrl.table1, margin = 1) %>% 
#   as.data.frame.matrix()
# print.data.frame(sbrl.table3, digits = 2)
